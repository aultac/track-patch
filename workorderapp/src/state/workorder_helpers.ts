...
export async function computeSecondsOnRoadSegmentForVehicleOnDay({ seg, vehicleid, day, mode }: { seg: RoadSegment, vehicleid: number, day: string /* YYYY-MM-DD */, mode: string }): Promise<number> {
    if (!seg['Route (Ref)']) {
        info('No Route (Ref)')

...
// for the 
export function guessRoadType(road: RoadNameProperties): RoadTypeInfo {
...
export function roadNameToType(geofulladdress: string): RoadTypeInfo {
  ...
  let ret: RoadTypeInfo = { name: geofulladdress, type: 'UNKNOWN' };
...
function isRamp(str: string) {
  if (!str.match(reFirstFullNumber)) return false; // If no number, it's not a ramp.

  if (str.match(ramp)) return true;
  if (str.match(numberWithExitLetter)) return true;
  if (str.match(endsWithExitLetter)) return true;
  return false;
}
